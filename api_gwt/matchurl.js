...
class URLPattern {
    constructor (pattern, mode, modifiers) {
        ...
        if (typeof pattern == 'string') {
            ...
            throw new Error('[URLPattern] The \'pattern\' property must be a string');
        }
        ...
        if (validModes.indexOf(mode) >= 0) {
            ...
            throw new Error(`[URLPattern] The 'mode' property must be one of ${validModes.join(', ')}`);
        }
        ...
        if (modifiers) {
            ...
            modifiers.split('').forEach((mod, pos, arr) => {
                if (validModifiers.indexOf(mod) < 0) {
                    throw new Error(`[URLPattern] Invalid modifier character '${mod}'`);
                }

                if (arr.indexOf(mod) != pos) {
                    throw new Error(`[URLPattern] Duplicate modifier character '${mod}'`);
                }
            });
    ...
    stick (obj) {
        ...
        switch (this.mode) {
            ...
            default: {
                throw new Error(`[URLPattern] Cannot call replace method on '${this.mode}' type`);
            }
        }
...
module.exports = function (pattern, mode, modifiers) {
    return new URLPattern(pattern, mode, modifiers);
};
